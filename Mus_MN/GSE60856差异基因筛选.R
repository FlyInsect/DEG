#####################################获得芯片表达矩阵,a1#########################################
library(GEOquery)
gset = getGEO('GSE60856',destdir = '.',AnnotGPL = 'F',getGPL = 'F')#将GSE20589数据下载到R里当前工作目录并赋值给gset
#class(gset) 
...
group_list <-c(rep('sod1_mutant',7),rep('control',7))



#limma分析前可能需对数据log2处理(下载矩阵的标准化处理方法MAS5返回值未经过log2转换)
#（该芯片数据预处理方法为GC-RMA，已取对数）
#new_a1 <- log2(new_a1)
...
library(pheatmap)
choose_gene=head(rownames(new_nrDEG_nor),109) #取多少基因做热图？
